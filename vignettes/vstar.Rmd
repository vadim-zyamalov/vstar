...
Let's generate another two series using only \(A_1\) and \(\Phi_1\) to compare the results of linearity tests. This new data has only one regime!

```{r}
x0 <- matrix(rnorm(2), nrow = 1)
x0 <- cbind(x0, 1)
...
Lets run a linearity tests. First we'll test `model` with 1 lag of the transition variable.

```{r}
linearity.test(model, J = 1)
```

And let's compare the result with the one of testing `model0`.
