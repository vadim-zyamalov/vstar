...
vstar.girf <- function(model,
  ...
  results <- foreach(
    ...
    for (r in 1:R) {
      tmp.eps <- matrix(
        boot.Ksi[sample(prod(dim(boot.Ksi)),
          size = k * (n + 1),
          replace = TRUE
        )],
        ncol = k
      ...
      for (j in 1:(n + 1)) {
        ...
        boot.G <- G.func(
          t(unity(m - 1)) %x% iter.trans[j],
          model$g,
          model$c
        )
        boot.G.p <- G.func(
          t(unity(m - 1)) %x% iter.trans.p[j],
          model$g,
          model$c
        )
        boot.G.m <- G.func(
          t(unity(m - 1)) %x% iter.trans.m[j],
          model$g,
          model$c
        )
        ...
        step.xr.p <- matrix(
          c(
            if (!is.null(iter.z)) iter.z[j, ] else NULL,
            step.xr.p,
        ...
        tmp.yr.p <- t(PSI.p) %*% t(model$coef) %*% t(step.xr.p) + tmp.eps.p[j, ]
        step.yr.p <- rbind(step.yr.p, t(tmp.yr.p))

        tmp.yr.m <- t(PSI.m) %*% t(model$coef) %*% t(step.xr.m) + tmp.eps.m[j, ]
        step.yr.m <- rbind(step.yr.m, t(tmp.yr.m))

        tmp.yr <- t(PSI) %*% t(model$coef) %*% t(step.xr) + tmp.eps[j, ]
        ...
        if (endo.trans) {
          if (trans.lag == 0) {
            ...
            if (j - trans.lag <= 0) {
              iter.trans.p <- c(
                iter.trans.p,
                model$data$Y[history, model$params$endo == trans][N - trans.lag + j] # nolint
              )
              iter.trans.m <- c(
                iter.trans.m,
                model$data$Y[history, model$params$endo == trans][N - trans.lag + j] # nolint
              ...
              iter.trans.p <- c(
                iter.trans.p,
                step.yr.p[j - trans.lag, model$params$endo == trans]
              )
              iter.trans.m <- c(
                iter.trans.m,
                step.yr.m[j - trans.lag, model$params$endo == trans]
              )
              iter.trans <- c(
                iter.trans,
                step.yr[j - trans.lag, model$params$endo == trans]
              )
            }
          }

          if (diff.trans) {
            iter.trans.p[j + 1] <- iter.trans.p[j + 1] + iter.trans.p[j]
            iter.trans.m[j + 1] <- iter.trans.m[j + 1] + iter.trans.m[j]
            iter.trans[j + 1] <- iter.trans[j + 1] + iter.trans[j]
          }
        }
      }
  ...
  for (i in 1:k) {
    v <- names.y[i]
    pos[[v]] <- qirf.p[((i - 1) * (n + 1) + 1):(i * (n + 1)), , drop = FALSE]
    rownames(pos[[v]]) <- 0:n
    neg[[v]] <- qirf.m[((i - 1) * (n + 1) + 1):(i * (n + 1)), , drop = FALSE]
    rownames(neg[[v]]) <- 0:n
  }
